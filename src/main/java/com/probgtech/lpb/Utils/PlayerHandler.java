...
public class PlayerHandler {
	...
	public static void leaveGame(Player p){
		...
		for (Item i :Items.items.values()) inv.remove(i.getItemStack()); 
	...
	public static void joinGame(Player p){
		...
		p.sendTitle(ChatColor.GOLD + "Welcome to Paintball", ChatColor.DARK_AQUA + "Made for TownCraft.us", 2, Timer, 2);
		PlayerInventory inv = p.getInventory();
		...
		for (int i = 0; i < 35; i++){
			if (Paintball.cfg.isSet("Layout."+i) && !Paintball.cfg.getString("Layout."+i).equalsIgnoreCase("optin")){
				int i1 = Paintball.cfg.getInt("Layout."+i);
				if (Items.getItems().get(i1).permission != null && p.hasPermission(Items.getItems().get(i1).permission)){
					inv.setItem(i, Items.getItems().get(i1).getItemStack());
				}
				else inv.setItem(i, Items.getItems().get(i1).getItemStack());
			}
		}
	...
	public static int getOptSlot(){
		for (int i = 0; i < 9; i++) if (Paintball.cfg.isSet("Layout"+i) && Paintball.cfg.getString("Layout"+i).equalsIgnoreCase("optin")) return i;
		return 8;
	}
