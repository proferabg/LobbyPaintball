    ...
    public boolean onCommand(CommandSender sender, Command command, String commandLabel, String[] args){
		if (args.length == 0){
    		...
    		{	
    			...
    			sender.sendMessage(ChatColor.RED + "/lpb about" + ChatColor.GRAY + " - Shows probgtech's bragging rights!");
    		}
    		else if (args[0].equalsIgnoreCase("about"))
    		{
    			sender.sendMessage(ChatColor.GRAY + " ------ " + ChatColor.RED + "LobbyPaintball" + ChatColor.GRAY + " ------ ");
    			sender.sendMessage(ChatColor.RED + "LobbyPaintball v" + plugin.getDescription().getVersion() + ChatColor.GRAY + " by proferabg");
