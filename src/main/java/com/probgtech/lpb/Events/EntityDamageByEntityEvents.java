...
import com.probgtech.lpb.Utils.MessageHandler;
...
public class EntityDamageByEntityEvents implements Listener {
	...
	public void onEntityDamage(EntityDamageByEntityEvent e) {
		...
		if (attacker instanceof Snowball && damaged instanceof ItemFrame) e.setCancelled(true);

		if (attacker instanceof Player && damaged instanceof Player){
			Player p = (Player) attacker;
			for (Item i : Items.getItems().values()){
				if (!p.getItemInHand().equals(i.getItemStack())) if (!p.hasPermission("lpb.staff")) { e.setCancelled(true); }

				if (i.getType() == ItemType.STICK && i.getPermission() != null && p.hasPermission(i.getPermission())){
					e.setDamage(0);
					e.setCancelled(false);
				} else if (!p.hasPermission("lpb.staff")) e.setCancelled(true);
			}
		}
